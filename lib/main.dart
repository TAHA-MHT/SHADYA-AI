...
class ShadyaAgentBridge {
  ...
  // Lance une application installée directement par son nom de package
  // (ex: "com.whatsapp"), via l'intent de lancement standard Android —
  // plus fiable qu'un schéma d'URL personnalisé (ex: "whatsapp://"), qui
  // n'est pas toujours défini par l'app cible pour une ouverture "à vide"
  // sans destination précise. Retourne true si le lancement a réussi.
  static Future<bool> lancerApplication(String packageName) async {
    try {
      final resultat = await platform.invokeMethod('lancerApplication', {
...
class _VoiceHomeScreenState extends State<VoiceHomeScreen> {
 ...
 Future<String?> _cheminModeleSiPresent() async {
  ...
  Future<bool> _gererFluxFacebook(String texte) async {
    ...
    // Mot d'annulation valable à tout moment pendant le flux
  ...
  Future<void> _finaliserFluxFacebook(String telephone, String age, String genre) async {
    ...
    try {
      final resultatLancement = await ShadyaAgentBridge.lancerApplication('com.facebook.katana');
      if (!resultatLancement) {
  ...
  Future<bool> _essayerOuvrirApplication(String texte) async {
    ...
    // Correspondance nom d'app → nom de package Android, utilisée pour
    // lancer l'application installée directement (voir
    // ShadyaAgentBridge.lancerApplication), plutôt que de dépendre d'un
    // schéma d'URL personnalisé (ex: "whatsapp://"). C'est ce dernier qui
    // faisait échouer l'ouverture de WhatsApp : canLaunchUrl renvoyait false
    // car WhatsApp n'expose pas d'intent-filter pour une ouverture "à vide"
    // (sans numéro/destination précise) via ce schéma, provoquant un repli
    // silencieux vers le navigateur web à la place de l'application.
    ...
    for (final entry in appsSchemes.entries) {
      if (texteMinuscule.contains(entry.key)) {
        ...
        final nomPackage = appsPackages[nomApp];
        if (nomPackage != null) {
          final lance = await ShadyaAgentBridge.lancerApplication(nomPackage);
