...
class FacebookAutomationHandler(private val service: AccessibilityService) {
    ...
    private fun handleSignup(rootNode: AccessibilityNodeInfo) {
        ...
        // par SMS est normalement déjà rempli par tenterRemplirCodeConfirmation
        // (déclenchée dès la réception de la notification SMS, voir
        // ShadyaAgentService). Ce bloc couvre le cas où l'écran de
        // confirmation apparaît APRÈS que le code a déjà été capturé (ou
        // avant, cas le plus fréquent, où le code n'est pas encore arrivé —
        // on journalise alors une simple attente plutôt que de spammer
        // "Next" sur un champ vide).
        ...
        if (champsCodeConfirmation.isNotEmpty()) {
            ...
            if (codeMemorise.isNotEmpty()) {
                ...
                journaliser("OTP: champ de confirmation détecté, en attente du SMS — aucune action")
            }
            return
        }
