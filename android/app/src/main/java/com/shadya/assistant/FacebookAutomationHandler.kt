...
class FacebookAutomationHandler(private val service: AccessibilityService) {
    ...
    // Évite de journaliser en boucle "clic suspendu" à chaque événement tant
    // que l'indicateur de chargement reste affiché — une ligne au début du
    // chargement suffit.
    ...
    // à l'écran (icône ou bouton avec texte/contentDescription "Loading"/
    // "Loading..."/"Chargement"). Utilisé pour suspendre tout clic
    // automatique tant qu'une requête réseau est déjà en cours côté
    // Facebook : cliquer "Next" en boucle pendant ce chargement (observé en
    // conditions réelles : des dizaines de clics répétés sur un bouton
    // encore en attente de réponse serveur) risque de faire traiter
    // plusieurs soumissions du même numéro par Facebook, ce qui pourrait
    // expliquer les cas où plusieurs codes de confirmation différents
    // avaient été reçus coup sur coup lors d'un test précédent.
    ...
    private fun clickNextButton(rootNode: AccessibilityNodeInfo) {
        if (estEnChargement(rootNode)) {
            if (!chargementDejaSignale) {
                journaliser("CHARGEMENT: indicateur de chargement détecté → clic suspendu, attente de la réponse serveur")
    ...
    // (observé sur WhatsApp : un vrai champ EditText n'était pas reconnu
    // comme éditable et se retrouvait exclu, empêchant tout remplissage).
    // On accepte désormais aussi tout nœud de classe EditText, même si le
    ...
    private fun findFieldsByHint(rootNode: AccessibilityNodeInfo, hints: List<String>): List<AccessibilityNodeInfo> {
        ...
        for (hint in hints) {
            ...
            if (!nodes.isNullOrEmpty()) {
                result.addAll(nodes.filter { it.isEditable || it.className == "android.widget.EditText" })
            }
        }
        return result
    }
