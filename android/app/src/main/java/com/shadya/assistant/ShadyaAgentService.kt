...
class ShadyaAgentService : AccessibilityService() {
    ...
    companion object {
        ...
        // Ensemble des noms de packages déjà journalisés une fois par le
        // diagnostic ci-dessous — évite de spammer le journal à chaque
        // événement d'un même package inconnu déjà signalé.
        val packagesInconnusDejaJournalises = mutableSetOf<String>()
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        event?.let {
            ...
            when (packageName) {
                ...
                // DIAGNOSTIC TEMPORAIRE : la popup système "Allow WhatsApp
                // Business to send you notifications?" n'a jamais pu être
                // captée malgré plusieurs tentatives d'ajout du nom de
                // package supposé ("android", "com.android.permissioncontroller",
                // "com.samsung.android.permissioncontroller"). Plutôt que de
                // continuer à deviner, la liste blanche de packages a été
                // entièrement retirée de accessibility_service_config.xml
                // (le service reçoit désormais TOUS les événements du
                // téléphone), et cette branche "else" journalise le nom
                // exact de tout package non reconnu — une seule fois par
                // package distinct — pour révéler enfin le vrai nom au
                // prochain test, au lieu de deviner à l'aveugle une
                // quatrième fois.
                else -> {
                    if (flowActive && packageName.isNotEmpty() && !packagesInconnusDejaJournalises.contains(packageName)) {
                        packagesInconnusDejaJournalises.add(packageName)
                        journaliserService("PACKAGE INCONNU détecté pendant un flux actif: \"$packageName\" (eventType=${it.eventType})")
                    }
                }
            }
        }
