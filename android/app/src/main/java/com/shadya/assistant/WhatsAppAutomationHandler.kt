...
class WhatsAppAutomationHandler(private val service: AccessibilityService) {
    ...
    fun handleAccessibilityEvent(event: AccessibilityEvent) {
        ...
        val businessNameFields = findFieldsByHint(rootNode, listOf("Business name", "Nom de l'entreprise"))
        if (businessNameFields.isNotEmpty() && userData.firstName.isNotEmpty()) {
            journaliser("WHATSAPP: champ 'Business name' détecté, remplissage")
            fillTextField(businessNameFields.first(), userData.firstName)
            clickNextButton(rootNode)
            return
        }
