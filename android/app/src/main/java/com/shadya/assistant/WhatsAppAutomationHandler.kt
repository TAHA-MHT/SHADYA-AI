...
class WhatsAppAutomationHandler(private val service: AccessibilityService) {
    ...
    fun handleAccessibilityEvent(event: AccessibilityEvent) {
        ...
        // catégorie d'entreprise (jusqu'à 3 possibles, une seule suffit
        // pour activer le bouton "Next", grisé tant qu'aucune n'est
        // sélectionnée). On sélectionne systématiquement "Other business"
        // (générique, s'applique à toute activité) plutôt que d'essayer de
        // deviner une catégorie pertinente à partir des données vocales.
        val estEcranCategorie = texteVisibleContient(rootNode, listOf("Select your business category"))
        if (estEcranCategorie) {
            val boutonCategorie = findNodesByText(rootNode, listOf("Other business")).firstOrNull { noeud ->
                val texte = noeud.text?.toString()?.trim()
                val description = noeud.contentDescription?.toString()?.trim()
