...
class WhatsAppAutomationHandler(private val service: AccessibilityService) {
    ...
    fun handleAccessibilityEvent(event: AccessibilityEvent) {
        ...
        // 4ter. Écran "Create your business profile" — champ "Business
        // name" à remplir. Contrairement aux autres champs, le libellé
        // "Business name" visible à l'écran est un simple TextView séparé
        // (pas le hint du champ éditable lui-même), donc la recherche par
        ...
        val estEcranBusinessProfile = findNodesByText(rootNode, listOf("Create your business profile")).isNotEmpty()
            ...
            val nomEntreprise = userData.businessName.ifEmpty { userData.firstName }
            if (champBusinessName != null && nomEntreprise.isNotEmpty()) {
                journaliser("WHATSAPP: champ 'Business name' détecté, remplissage avec \"$nomEntreprise\"")
                fillTextField(champBusinessName, nomEntreprise)
                clickNextButton(rootNode)
            } else {
                journaliser("WHATSAPP: écran 'Create your business profile' détecté mais champ introuvable (trouvé=${champBusinessName != null}) ou nom vide")
            }
            return
        }
    ...
    // Recherche récursive de tous les champs éditables de l'écran — utilisé
    // en repli quand la recherche par hint textuel échoue (champ vide sans
    // texte exploitable, comme "Business name").
