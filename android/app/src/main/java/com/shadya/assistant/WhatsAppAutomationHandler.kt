...
class WhatsAppAutomationHandler(private val service: AccessibilityService) {
    ...
    // NOUVEAU : Cooldown anti-rafale. Observé en conditions réelles : le
    // bot cliquait sur "Skip" toutes les 15-20 ms en continu (plusieurs
    // dizaines de clics par seconde), ce qui semble avoir corrompu l'état
    // interne de WhatsApp Business (blocage persistant sur "Enter your
    // name to continue" malgré des clics "réussis"). On impose désormais
    // un délai minimum entre deux traitements d'événement, pour se
    // rapprocher d'un rythme de clic humain et laisser à WhatsApp le temps
    // de traiter chaque action avant la suivante.
    private var dernierTraitementTimestamp: Long = 0
    private val delaiMinimumEntreTraitementsMs = 700L
    ...
    fun handleAccessibilityEvent(event: AccessibilityEvent) {
        ...
        // Cooldown anti-rafale : si le dernier traitement remonte à moins
        // de 700ms, on ignore complètement cet événement (on ne fait ni
        // détection ni clic). Ça évite la boucle de clics ultra-rapides
        // observée sur "Skip" (Add a business description).
        val maintenant = System.currentTimeMillis()
        if (maintenant - dernierTraitementTimestamp < delaiMinimumEntreTraitementsMs) {
            return
        }
        ...
        if (estPopupChoixNumeroGoogle) {
            ...
            if (boutonFermer != null) {
                ...
                dernierTraitementTimestamp = System.currentTimeMillis()
            } else {
