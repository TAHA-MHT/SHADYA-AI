...
class WhatsAppAutomationHandler(private val service: AccessibilityService) {
    ...
    fun handleAccessibilityEvent(event: AccessibilityEvent) {
        ...
        // 5sexies. Écran "Add a profile picture" — proposition d'ajouter
        // une photo de profil (non automatisé pour l'instant). "Next" reste
        // grisé sans photo, donc on clique sur "Skip" (en haut à droite)
        // pour passer cette étape, cohérent avec le choix fait pour la
        // photo de profil sur Facebook.
        ...
        // 5octies. Écran "Add a business description" — DIAGNOSTIC
        // TEMPORAIRE : boucle infinie observée (Skip cliqué avec succès en
        // continu, mais WhatsApp affiche "Enter your name to continue" et
        // ne progresse jamais). On dump le contenu complet de l'écran au
        // lieu de cliquer, pour identifier ce qui bloque réellement.
        val estEcranDescription = texteVisibleContient(rootNode, listOf("Add a business description"))
        if (estEcranDescription) {
            journaliser("WHATSAPP DIAGNOSTIC: écran 'Add a business description' — dump complet avant tout clic")
            val lignesDump = mutableListOf<String>()
            collecterContenuTexte(rootNode, lignesDump)
            for (ligne in lignesDump) {
                journaliser("  $ligne")
            }
            return
        }
