...
class WhatsAppAutomationHandler(private val service: AccessibilityService) {
    ...
    fun handleAccessibilityEvent(event: AccessibilityEvent) {
        ...
        if (estEcranChoixVerification) {
            ...
            // Les boutons radio de cet écran n'ont ni texte ni description
            // (confirmé par diagnostic : 3 éléments checkable trouvés, tous
            // avec texte="null" et desc="null") — le libellé "Receive SMS"
            // vit sur un élément VOISIN (frère), pas sur le bouton lui-même
            // ni sur un ancêtre/descendant. On fait donc correspondre le bon
            // bouton radio par PROXIMITÉ VERTICALE à l'écran : celui dont le
            // centre est le plus proche du centre du texte "Receive SMS",
            // en supposant qu'ils appartiennent à la même ligne visuelle.
        ...
        // MODIFIÉ : détection via texteVisibleContient (parcours manuel de
        // l'arbre) plutôt que findNodesByText/findAccessibilityNodeInfosByText,
        // cette dernière s'étant révélée par moments incapable de retrouver
        // ce texte pourtant bien présent (confirmé par le dump), provoquant
        // un blocage silencieux sur cet écran.
        ...
        // popups de permissions système (Contacts, Médias) non automatisées,
        // cohérent avec le choix fait pour la photo de profil sur Facebook.
        ...
        // 4ter. Écran de restauration de sauvegarde Google Drive — "If you
        // previously backed up to Google storage...". On clique sur "Skip"
        // (jamais "Give permission", qui ouvrirait une cascade de
        // permissions Google supplémentaires non préparées) : un nouveau
        // compte n'a de toute façon aucune sauvegarde existante à restaurer.
        ...
        // 4quater. Écran "Restore a backup" — écran DISTINCT du précédent :
        // texte différent ("restore your backup from Google storage" vs
        // "backed up to Google storage") et boutons différents
        // ("Cancel"/"Continue" au lieu de "Skip"). On clique sur "Cancel"
        // pour repartir sur un compte neuf sans tenter de restaurer une
        ...
        if (estEcranRestoreBackup) {
            val boutonCancel = findNodesByText(rootNode, listOf("Cancel", "Annuler")).firstOrNull { noeud ->
                val texte = noeud.text?.toString()?.trim()
                val description = noeud.contentDescription?.toString()?.trim()
        ...
        // 5bis. Écran "Create your business profile" — demande un nom
        // d'entreprise ("Business name"). On y met le prénom dicté par
        // l'utilisateur, faute d'un nom d'entreprise distinct collecté par
        // le flux vocal actuel.
        // MODIFIÉ : détection de l'écran via texteVisibleContient (comme
        // pour "Verified" ci-dessus), et repli sur le premier champ
        // éditable trouvé (collecterChampsEditables) si la recherche par
        // hint échoue — le libellé "Business name" étant un TextView séparé
        // du champ de saisie lui-même sur cet écran.
    ...
    // MODIFIÉ : recherche manuelle (rechercherManuel, comme
    // texteVisibleContient) au lieu de findNodesByText, cette dernière
    // s'étant révélée peu fiable à plusieurs reprises — elle ne retrouvait
    // pas le bouton "Next" pourtant bien présent après remplissage du champ
    // "Business name", empêchant tout clic après remplissage.
    ...
    // NOUVEAU : Recherche manuelle récursive de texte visible n'importe où
    // dans l'écran — utilisée en remplacement de
    // findAccessibilityNodeInfosByText pour la DÉTECTION d'écran, celle-ci
    // s'étant révélée peu fiable à plusieurs reprises (ex: écran "Verified"
    // ou "Business name" non détectés malgré un texte confirmé présent par
    // ailleurs, via le dump).
    ...
    // NOUVEAU : Recherche récursive de tous les champs éditables de l'écran
    // — utilisé en repli quand la recherche par hint textuel échoue (champ
    // vide sans texte exploitable, comme "Business name").
